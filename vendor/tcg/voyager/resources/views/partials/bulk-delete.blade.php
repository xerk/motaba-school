            ...
            <div class="modal-footer">
                <form action="{{ route('voyager.'.$dataType->slug.'.index') }}/0" id="bulk_delete_form" method="POST">
                    ...
                    <input type="submit" class="btn btn-danger pull-right delete-confirm"
                             value="{{ __('voyager::generic.bulk_delete_confirm') }} {{ Auth::user()->settings['locale'] == 'ar' ? $dataType->display_name_ar_plural : strtolower($dataType->display_name_plural) }}">
                </form>
...
window.onload = function () {
    ...
    $bulkDeleteBtn.click(function () {
        ...
        if (count) {
            ...
            $bulkDeleteInput.val('');
            // Deletion info
            var displayName = count > 1 ? '{{ Auth::user()->settings["locale"] == "ar" ? $dataType->display_name_ar_plural : $dataType->display_name_plural }}' : '{{ Auth::user()->settings["locale"] == "ar" ? $dataType->display_name_ar_singular : $dataType->display_name_singular }}';
