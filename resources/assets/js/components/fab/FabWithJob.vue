        ...
        <modal v-if="showModal" @close="showModal = false">
            <div slot="body">
               <div class="form-group">
                    <label for="gender">{{ trans('fab.Subject')}}Job</label>
                    <select class="form-control select2" id="job" v-model="jobSelect" @change="changeJob" name="job">
                        <option value="">{{ trans('fab.Choose Job')}}</option>
                        <option :value="true">{{ trans('fab.Student')}}</option>
                        <option :value="false">{{ trans('fab.Employee')}}</option>
                    </select>
                </div>
                <div v-show="jobSelect == true">
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            if (localStorage.job) {
                this.jobSelect = localStorage.job;
            }
        },
        computed: {
            classEduFilter() {
