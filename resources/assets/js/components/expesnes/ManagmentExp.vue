									...
									<div class="col-md-4">
										...
										>
											{{
												...
												parseInt(user.costSum) +
												parseInt(user.indebtednessSum) -
												(parseInt(user.paySum) +
													parseInt(user.discountSum))
													? parseInt(
															user.class_edu
																.expenses_cost
													  ) +
			...
			<div class="col-md-9">
				<div>
					<a
						...
						:title="trans('expenses.Make Payment')"
						><i class="voyager-credit-card"></i>
						{{ trans("expenses.Make Payment") }}</a
					>
				</div>
						...
						<div class="table-responsive">
							...
							>
								<thead>
									<tr>
										<th>
											{{ trans("expenses.Created At") }}
										</th>
										<th>
											{{
												trans("expenses.Serial Number")
											}}
										</th>
										<th>
											{{ trans("expenses.Pay date") }}
										</th>
										<th>{{ trans("expenses.Pay") }}</th>
										<th>{{ trans("expenses.Cost") }}</th>
										<th>
											{{ trans("expenses.Discount") }}
										</th>
										<!-- <th>{{ trans('expenses.Bus Expenses') }}</th> -->
										<th>
											{{ trans("expenses.Indebtedness") }}
										</th>
								...
								<tbody>
									<tr
										v-for="(item, index) in user.expenses"
										:key="index"
									>
										<td>
											{{ item.created_at }}
			...
			<div slot="body">
				<div
					:class="{
						...
						'has-error': errors.has('serial_number'),
					}"
				>
					...
					<input
						...
						type="text"
						:placeholder="trans('expenses.Serial Number')"
						v-validate="'required'"
						class="form-control"
						name="serial_number"
					/>
				...
				>
					...
					/>
					<span
						v-show="errors.has('pay_date')"
						class="help-block"
						style="color: #f96868"
						>{{ errors.first("pay_date") }}</span
					>
				</div>
				<div
					:class="{
						'form-group col-md-6': true,
						'has-error': errors.has('pay'),
					}"
				>
					<label for="pay">{{
						trans("expenses.Payment Amount")
					}}</label>
					<input
					...
					/>
					<span
						v-show="errors.has('pay')"
						class="help-block"
						style="color: #f96868"
						>{{ errors.first("pay") }}</span
					>
				</div>
				...
				<div
					:class="{
						'form-group col-md-6': true,
						'has-error': errors.has('indebtedness'),
					}"
				>
					<label for="indebtedness">{{
						trans("expenses.Indebtedness")
					}}</label>
					<input
						v-model="model.indebtedness"
						type="number"
						v-validate="'numeric|required'"
						class="form-control"
						name="indebtedness"
					/>
					<span
						v-show="errors.has('indebtedness')"
						class="help-block"
						style="color: #f96868"
						>{{ errors.first("indebtedness") }}</span
					>
				</div>
				<div
					:class="{
						'form-group col-md-6': true,
						'has-error': errors.has('cost'),
					}"
				>
	...
	export default {
		created() {
			...
			recaptchaScript.setAttribute(
				"src",
				"https://old.schoolver.com/js/lang.js"
			);
