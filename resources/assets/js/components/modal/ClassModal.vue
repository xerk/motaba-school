            ...
            <div class="modal-wrapper">
                <div class="modal-container">
                    ...
                    <div class="modal-body">
                        <slot name="body">
                            <div :class="{'form-group col-md-12': true, 'has-error': errors.has('stage') }">                            
                                <label for="name">{{ trans('configs.Stage education') }}</label>
                                <select required class="form-control select2" v-validate="'required'" v-model="itemClass.stage_edu_id.$model">
                            ...
                            <div :class="{'form-group col-md-12': true, 'has-error': errors.has('edu-system') }">                            
                                <label for="name">{{ trans('configs.Educational System') }}</label>
                                <select required class="form-control select2" v-validate="'required'" v-model="itemClass.egy_edu_system_id.$model">
                                ...
                                <span v-show="errors.has('edu-system')" class="help-block" style="color:#f96868">{{ errors.first('edu-system') }}</span>
                            </div>
                            <div :class="{'form-group col-md-12': true, 'has-error': errors.has('name') }">
                                <label for="name">{{ trans('configs.Name') }}</label>
                                <input required v-model="itemClass.$model.name" type="text" v-validate="'required'" class="form-control" name="name">
...
export default {
    ...
    data() {
        return {
            ...
            post: {
                apiURL: 'system-configs-update-class',
            },
            stageEdu: '',
            eduSystem: '',
        }
                ...
                if (result) {
                    this.$store.dispatch('submitForm', {
                        ...
                        .then(response => {
                            ...
                            this.$emit('close')
                            this.$emit('update', this.itemClass)
                        })
                }
            })
        },
