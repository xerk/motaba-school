			...
			<div class="panel panel-bordered">
				<div class="panel-body">
					<h3 class="text-center">
						إحصائية
						{{ status != "all" ? status.name : "جميع الحالات" }}
					</h3>
					<h3 class="text-center">
						<span v-if="classEdu != ''">{{
			...
			fetch() {
				...
				$(function () {
					var t = $("#users-table").DataTable({
						...
						processing: false,
						serverSide: false,
						ajax: `https://old.schoolver.com/admin/get-new-status?class=${classEdu}&classroom=${classRoom}&status=${
							status == "all" ? status : status.id
						}`,
						columns: [
