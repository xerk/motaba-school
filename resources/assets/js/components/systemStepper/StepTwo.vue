        ...
        <div class="row">
            ...
            <div v-for="(option, key) in $v.form.options.$each.$iter" :key="key">
                <div :class="['form-group col-md-3', (option.$error) ? 'has-error' : '']">
                    ...
                    <span v-if="option.$error" class="help-block" style="color:#f96868">{{ trans('configs.This stage is invalid')}}</span>
                </div>
                <div :class="['form-group col-md-4', (option.$error) ? 'has-error' : '']">
                    ...
                    <span v-if="option.$error" class="help-block" style="color:#f96868">{{ trans('configs.This education system is invalid')}}</span>
                </div>
                <div :class="['form-group col-md-4', (option.$error) ? 'has-error' : '']">
                    <input required="" type="text" class="form-control" v-model.trim="option.name.$model"  :placeholder="trans('configs.Type name of class edu')" value="">
                    <span v-if="option.$error" class="help-block" style="color:#f96868">{{ trans('configs.This class education is invalid')}}</span>
                </div>
                <div class="form-group col-md-1">
                    ...
                    <modal v-if="showModal" @close="showModal = false">
                        ...
                        default content
                        -->
                        <h3 slot="header"><i class="voyager-trash"></i> {{ trans('configs.Are you sure you want to delete this class?')}}</h3>                        
            ...
            <div class="form-group col-md-12">
                ...
                <a @click="addOption" class="btn btn-success">+ {{ trans('configs.Add New Class Edu')}}</a>
            </div>
        </div>
    </div>
</template>
