<template>
	<div class="page-content browse container-fluid">
			...
			<div class="content-header" style="display: none">
				<div class="row">
					...
					<div class="col-print-4 text-center">
						<h3 class="text-center" v-if="user">
							<span
								>{{ user.name }} {{ user.last_name }} سجل
								راتب</span
							>
						</h3>
					</div>
			...
			<div class="content-footer" style="display: none">
				...
				<span
					...
					><img
						...
						alt="Logo"
						style="width: 32px; display: inline; margin-right: 5px"
						class="img-responsive"
					/>
								...
								<div class="row">
									...
									<div class="col-md-4">
										<span
											...
											class="show-result label label-primary"
										>
											{{
												(cost =
													addSalary.cost +
													addSalary.bonus +
													addSalary.may_grant +
													addSalary.prev_year_bonus +
													addSalary.share_employer +
													addSalary.share_employee +
													addSalary.variable_wages)
											}}
										</span>
									</div>
								</div>
								...
								<div class="row">
									<div class="col-md-8">
										<span class="show-field"
											>{{
												trans(
													"salary.Absence of patients"
												)
											}}
											-
											{{
												Math.round(
													addSalary.absent_day / 4,
													2
												)
											}}:</span
										>
							...
							<div class="panel-body">
								<div class="row">
									...
									<div class="col-md-4">
										<span
											style="font-size: 14px"
											class="show-result label label-success"
										>
											{{
												parseInt(cost) +
												parseInt(allowances) -
								...
								<div class="row">
									<div class="col-md-8">
										<span class="show-field"
											>{{
												trans(
													"salary.Total absence of patients"
												)
											}}:</span
										>
									</div>
								...
								<div class="row">
									<div class="col-md-8">
										<span class="show-field"
											>{{
												trans(
													"salary.Total delayed supervision"
												)
											}}:</span
										>
									</div>
								...
								<div class="row">
									<div class="col-md-8">
										<span class="show-field"
											>{{
												trans(
													"salary.Total delay permissions"
												)
											}}:</span
										>
									</div>
			...
			<div class="col-md-9">
				<div>
					<a
						...
						@click="paymentClickModal"
						:title="trans('salary.Pay Salary')"
						><i class="voyager-credit-card"></i>
						{{ trans("salary.Pay Salary") }}</a
					>
					<router-link
						@click.native="next"
					...
					<button
						class="btn btn-sm disabled pull-right"
						style="padding: 4px 15px"
					>
						{{ dateNow | moment("(MM) MMMM - YYYY") }}
					</button>
				</div>
				<div class="panel panel-bordered">
							...
							>
								<thead>
									<tr>
										...
										<th>
											{{
												trans("salary.Total Allowances")
											}}
										</th>
										<th>
											{{
												trans("salary.Total Deductions")
											}}
										</th>
								...
								<tbody>
									<tr
										v-for="(item, index) in user.attendance"
										:key="index"
									>
										<td>
											<span
											...
											<span
												...
												>{{
													trans(
														"attendance.Absence of patients"
													)
												}}</span
											>
											<span
												...
												>{{
													trans(
														"attendance.Delayed supervision"
													)
												}}</span
											>
											<span
						...
						<div class="table-responsive">
							...
							>
								<thead>
									<tr>
										<th>
											{{ trans("salary.Payment Method") }}
										</th>
										<th>
											{{ trans("salary.Net Salary") }}
										</th>
										<th>
											{{ trans("salary.Payment Amount") }}
										</th>
										<th>
											{{ trans("salary.Created At") }}
										</th>
										<th class="actions text-right">
											{{ trans("table.Actions") }}
										</th>
								...
								<tbody>
									<tr
										v-for="(item, index) in makePayments"
										:key="index"
									>
										<td>
											<span
		...
		>
			...
			<div slot="body">
				<div
					:class="{
						...
						'has-error': errors.has('payment-method'),
					}"
				>
					...
					>
						<option value="">
							{{ trans("salary.Choose Payment Method") }}
						</option>
					...
					<span
						...
						class="help-block"
						style="color: #f96868"
						>{{ errors.first("payment-method") }}</span
					>
				</div>
				<div
					:class="{
						'form-group col-md-6': true,
						'has-error': errors.has('net_salary'),
					}"
				>
					...
					/>
					<span
						v-show="errors.has('net_salary')"
						class="help-block"
						style="color: #f96868"
						>{{ errors.first("net_salary") }}</span
					>
				</div>
				<div
					:class="{
						'form-group col-md-6': true,
						'has-error': errors.has('payment-amount'),
					}"
				>
					...
					/>
					<span
						v-show="errors.has('payment-amount')"
						class="help-block"
						style="color: #f96868"
						>{{ errors.first("payment-amount") }}</span
					>
				</div>
			...
			<button slot="button" class="btn btn-success" @click="parsist">
				{{ trans("table.Yes, Save it!") }}
			</button>
		</make-payment>
		<salary
			@getAttend="fetch"
	...
	export default {
		...
		methods: {
			...
			paymentClickModal() {
				var tSalary =
					parseInt(this.cost) +
					parseInt(this.allowances) -
					(parseInt(this.deductions) +
						parseInt(this.addSalary.absent_day * this.absentCount) +
						parseInt(
							(this.addSalary.absent_day / 4) * this.patientCount
						));
			...
			fetch() {
				this.$store
					.dispatch("retrivePayemtnDetails", {
						get: this.get,
