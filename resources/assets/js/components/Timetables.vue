<template>
            ...
            <div class="col-md-12">
                <div class="panel panel-bordered">
                    <div class="panel-body">
                        <h3 class="timetable-header text-center"><span v-if="classEdu != ''">{{classRoomName.class_edu.name}}</span> - <span v-if="classRoom != ''">{{classRoomName.name}}</span></h3>
                        <div class="content-header" style="display: none">
                            <div class="row">
                                <div class="col-print-4 pull-left" style="padding-left: 10px">
                                    <img src="http://localhost/images/logo/PNG-24.png" alt="Logo" style="width:75px" class='img-responsive' />
                                </div>
                                <div class="col-print-4 text-center">
                                    <h3 class="text-center"><span v-if="classEdu != ''">{{classRoomName.class_edu.name}}</span> - <span v-if="classRoom != ''">{{classRoomName.name}}</span></h3>
                                </div>
                        ...
                        <div class="content-footer" style="display:none">
                            ...
                            <span style="position: fixed;bottom: 0;font-size: 12px;padding: 3px;font-weight: 400;"><img src="http://localhost/images/logo/motawer-black.png" alt="Logo" style="width:32px;display: inline;margin-right: 5px;" class='img-responsive' /> Powered by MOTAWER</span>
                        </div>
                        <hr>
                        <table dir="auto" class="table dataTable timetable-table" id="users-table">
                            ...
                            <tbody class="timetable-tbody">
                                <tr>
                                    <td>الأحد</td>
                                    <td v-for="(item, key) in timetableSun" :key="key" @click="openModal(item)" v-html="(item.subject ? item.subject.name : '<span></span>' )"></td>
                                </tr>
                                <tr>
                                    <td>الإثنين</td>
                                    <td v-for="(item, key) in timetableMon" :key="key" @click="openModal(item)" v-html="(item.subject ? item.subject.name : '<span></span>' )"></td>
                                </tr>
                                <tr>
                                    <td>الثلاثاء</td>
                                    <td v-for="(item, key) in timetableTues" :key="key" @click="openModal(item)" v-html="(item.subject ? item.subject.name : '<span></span>' )"></td>
                                </tr>
                                <tr>
                                    <td>الأربعاء</td>
                                    <td v-for="(item, key) in timetableWedens" :key="key" @click="openModal(item)" v-html="(item.subject ? item.subject.name : '<span></span>' )"></td>
                                </tr>
                                <tr>
                                    <td>الخميس</td>
                                    <td v-for="(item, key) in timetableThurs" :key="key" @click="openModal(item)" v-html="(item.subject ? item.subject.name : '<span></span>' )"></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
    ...
    <modal-timetable :selectRow="selectRow" :subjects="subjects" @confirm="editOption" v-if="modal" @close="modal = false"></modal-timetable>
    </div>
</template>
<script>
